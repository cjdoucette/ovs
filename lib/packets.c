...
void
packet_set_xia(struct dp_packet *packet, uint8_t version, uint8_t next_hdr, ovs_be16 payload_len, uint8_t hop_limit, uint8_t num_dst, uint8_t num_src, uint8_t last_node)
{
    struct xiphdr *xiphdr = dp_packet_l3(packet);
    xiphdr->version = version;
    xiphdr->next_hdr = next_hdr;
    xiphdr->payload_len = payload_len;
    xiphdr->hop_limit = hop_limit;
    xiphdr->num_dst = num_dst;
    xiphdr->num_src = num_src;
